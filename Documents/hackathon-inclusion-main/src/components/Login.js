...
const Login = () => {
  ...
  const validateForm = () => {
    // Always allow form submission - no validation
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        // Allow login with empty credentials - use defaults
        const email = formData.email || 'demo@example.com';
        const password = formData.password || 'demo123';
        
        const response = await login(email, password);
        // Redirect based on user type and primary identity
        const userType = response.user.user_type;
        const primaryIdentity = response.user.primary_identity;
        
        // Check for specific identities first
        if (primaryIdentity === 'pwd') {
          navigate('/pwd/dashboard');
        } else if (primaryIdentity === 'refugee') {
          navigate('/refugee/dashboard');
        } else if (primaryIdentity === 'lgbtq') {
          navigate('/lgbtq/dashboard');
        } else if (userType === 'admin' || userType === 'unilink_team') {
          navigate('/admin');
  ...
  const getUserTypeLabel = (userType) => {
    switch (userType) {
      ...
      case 'unilink_team': return t('userTypeHEVATeam');
